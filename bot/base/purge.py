    ...
    try:
        ...
        for t in scheduler.get_task_list() or []:
            try:
                ...
                attachment = None
                # uma-it tasks carry the same TaskDetail as umamusume ones -
                # its build_task delegates to the umamusume builder - so they
                # serialize the same way. Leaving it out is not a cosmetic
                # miss: the entry persists with attachment_data null, restoring
                # it raises inside build_task, load_saved_tasks swallows that
                # and drops the task, and the next persist writes the shortened
                # list back. That is how a task list becomes [].
                if app in ('umamusume', 'uma-it'):
