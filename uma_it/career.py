...
def script_wait(ctx):
    ...
    # Nothing else happens here. This handler used to hand the countdown over
    # to a team trials session, which spent the run's spare RP and came back;
    # RP accrues too slowly - a point every 90 minutes - for that to be worth
    # leaving a career for, so sessions now run between careers instead.
